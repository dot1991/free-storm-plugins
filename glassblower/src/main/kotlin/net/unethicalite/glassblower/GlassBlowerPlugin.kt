            ...
            when(getState()){
                ...
                States.HANDLE_BANK -> {
                    if (Bank.isOpen())
                    {
                        ...
                        if (!Inventory.contains(ItemID.GLASSBLOWING_PIPE) && Bank.contains(ItemID.GLASSBLOWING_PIPE))
                        {
                            Bank.withdraw(ItemID.GLASSBLOWING_PIPE, 1, Bank.WithdrawMode.ITEM)
                            return sleepDelay().toInt()
                        }
                        if (!Inventory.contains(ItemID.MOLTEN_GLASS) && Bank.contains(ItemID.MOLTEN_GLASS))
                        {
